        ...
        for f in range(F):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    out[n,f,h_out,w_out] = np.sum(x_pad[n,0,h_out*s:h_out*s+HH,w_out*s:w_out*s+WW]*w[f,0,:,:]
                                                 + x_pad[n,1,h_out*s:h_out*s+HH,w_out*s:w_out*s+WW]*w[f,1,:,:]
                                                 + x_pad[n,2,h_out*s:h_out*s+HH,w_out*s:w_out*s+WW]*w[f,2,:,:]) + b[f]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_reshape = np.reshape(x, (N*G, C//G*H*W))
...
def spatial_groupnorm_backward(dout, cache):
    ...
    normalized_x = np.reshape(normalized_x,(N*G, C//G*H*W))
    ...
    dx = (1 / (C//G*H*W)) * ivar * (((C//G*H*W) * dxhat) - np.sum(dxhat, axis=0) - normalized_x * np.sum(dxhat * normalized_x, axis=0))
    dx = dx.T
